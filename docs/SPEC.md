...
本文中で `[[用語]]` と書くと図の上でマウスオーバー説明（ツールチップ）が出る（§4.5 / §5.7）。
...
- どうしても同一セルに積む場合は `at: [役割, 時点, スロット番号]`（1始まり）で順序を固定する。
  このときレンダラは**装飾を省略してよい**（§5.6）。省略した内容は `.flow` に残るため情報は失われない。
...
- 凡例（§5.4）の項目名も同じ日本語を使う。
- 枝ラベルに説明を足して長くしない（置き場所が無くなる。§5.8）。
...
- ラベルは**短く**（`docs/SPEC.md §5.5` の語）。修飾語を足して長くしない（「はい（差異あり）」ではなく「はい」）。
...
- 図中で参照されていない `glossary:` の項目は、用語集カードに出さない（凡例と同じ考え方＝§5.4）。
