...
**https://business-flow-visualizer.vercel.app/**

実例ごとに「①ヒアリング → ②`.flow` → ③業務フロー図」を切り替えて確認できます。
ヒアリング記録は全文、`.flow` も全文を載せており、**どんな質問からどの記法フィールドが埋まったか**まで追えます。
業務フロー図は埋め込みと別タブの両方で開けます。

> ソースは [`index.html`](index.html)。図は `examples/` 配下を相対参照するため、
> 自前でホストする場合は**リポジトリのルートを配信対象**にしてください。
